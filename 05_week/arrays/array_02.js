...
wowDatatypes.foreaach((item, index) => {
  const dataType = typeof item;
  console.log(item, index, dataType);
});*/
...
// Write your code here
function commonCourses(student1Courses, student2Courses) {
  return student1Courses.filter((commonCourses) =>
    student2Courses.includes(commonCourses)
  );
}
console.log("Common course:" + commonCourses(student1Courses, student2Courses)); */
...
furniture.forEach((furniture) => {
  furniture.split("").forEach((letter) => {
    console.log("Letters in furniture" + letter);
  });
}); */
